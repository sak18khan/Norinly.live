                ...
                <div className="p-10 space-y-10 max-h-[75vh] overflow-y-auto no-scrollbar relative z-10">
                    {loading ? (
                        <div className="py-24 flex flex-col items-center justify-center gap-5">
                            ...
                            <p className="text-zinc-400 text-[10px] font-black uppercase tracking-[0.2em] animate-pulse">Retrieving Archives...</p>
                        </div>
                    ) : (
                        <div className="space-y-10">
                            {/* Stats Grid */}
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-black/5 border border-black/5 p-6 rounded-[2.5rem] text-center group hover:border-accent/30 hover:bg-black/[0.08] transition-all shadow-sm">
                                    <div className="text-foreground font-black text-2xl tracking-tighter leading-none">{stats.conversationsCount}</div>
                                    <div className="text-[9px] text-zinc-400 font-black uppercase tracking-[0.2em] mt-3 group-hover:text-accent transition-colors">Sessions</div>
                                </div>
                                <div className="bg-black/5 border border-black/5 p-6 rounded-[2.5rem] text-center group hover:border-positive-accent/30 hover:bg-black/[0.08] transition-all shadow-sm">
                                    <div className="text-foreground font-black text-2xl tracking-tighter leading-none">{stats.totalSpeakingMinutes}</div>
                                    <div className="text-[9px] text-zinc-400 font-black uppercase tracking-[0.2em] mt-3 group-hover:text-positive-accent transition-colors">Minutes</div>
                                </div>
                                <div className="bg-black/5 border border-black/5 p-6 rounded-[2.5rem] text-center group hover:border-accent/30 hover:bg-black/[0.08] transition-all shadow-sm">
                                    <div className="text-foreground font-black text-2xl tracking-tighter leading-none">{stats.countriesCount}</div>
                                    <div className="text-[9px] text-zinc-400 font-black uppercase tracking-[0.2em] mt-3 group-hover:text-accent transition-colors">Nations</div>
                                </div>
                            ...
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.25em] ml-2">Public Identity</label>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative group/input">
                                    <AtSign className="absolute left-5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-zinc-400 group-focus-within/input:text-accent transition-colors" />
                                    <input
                                        type="text"
                                        value={username}
                              ...
                              {/* Country */}
                             <div className="space-y-3">
                                 <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.25em] ml-2">Location</label>
                                 <input
                                     type="text"
                                     value={country}
                                     onChange={(e) => setCountry(e.target.value)}
                                     placeholder="e.g. United Kingdom"
                                     className="w-full bg-black/5 border border-black/5 rounded-[1.5rem] px-6 py-4.5 text-xs font-black uppercase tracking-widest text-foreground focus:outline-none focus:ring-2 focus:ring-accent/10 focus:border-accent/30 transition-all placeholder:text-zinc-300 shadow-sm"
                                 />
                             </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <select
                                    ...
                                    className="w-full bg-black/5 border border-black/5 rounded-[1.5rem] px-6 py-4.5 text-xs font-black uppercase tracking-widest text-foreground focus:outline-none focus:ring-2 focus:ring-accent/10 focus:border-accent/30 transition-all appearance-none cursor-pointer shadow-sm"
                                >
                                    {['Beginner', 'Intermediate', 'Advanced', 'Native'].map(level => (
                                        <option key={level} value={level} className="bg-white text-black">{level}</option>
                                    ))}
                                </select>
                            </div>
                            ...
                            {/* Learning Goals */}
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.25em] ml-2">Strategic Objectives</label>
                                <textarea
                                    value={goals}
                                    onChange={(e) => setGoals(e.target.value)}
                                    placeholder="Briefly describe your targets..."
                                    rows={2}
                                    className="w-full bg-black/5 border border-black/5 rounded-[1.5rem] px-6 py-4.5 text-xs font-black uppercase tracking-widest text-foreground focus:outline-none focus:ring-2 focus:ring-accent/10 focus:border-accent/30 transition-all resize-none placeholder:text-zinc-300 shadow-sm"
                                />
                            </div>

                            {/* Phone Number (Optional) */}
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.25em] ml-2">Communication Link (Optional)</label>
                                <div className="relative group/input">
                                    <Phone className="absolute left-5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-zinc-400 group-focus-within/input:text-accent transition-colors" />
                                    <input
                                        type="tel"
                                        value={phone}
                            ...
                            {/* Email (Read-only) */}
                            <div className="space-y-3">
                                <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.25em] ml-2">Verified Endpoint (Private)</label>
                                <div className="relative opacity-50">
                                    <Mail className="absolute left-5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-zinc-400" />
                                    <input
                                        type="email"
                                        value={email}
                                        disabled
                                        className="w-full bg-black/5 border border-black/5 rounded-[1.5rem] pl-14 pr-6 py-4.5 text-xs font-black text-foreground cursor-not-allowed"
                                    />
                                </div>
                            </div>
