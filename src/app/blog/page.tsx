            ...
            <main className="flex-grow pt-32 pb-24 md:pt-48 md:pb-40">
                <div className="max-w-7xl mx-auto px-6">
                    ...
                    <div className="text-center mb-16 md:mb-24 space-y-6">
                        ...
                        <p className="text-lg md:text-2xl text-secondary-text max-w-2xl mx-auto font-normal leading-relaxed">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-16">
                        <div className="flex items-center p-1 bg-surface border border-border rounded-2xl overflow-x-auto max-w-full">
                            {categories.map(category => (
                                <button
                                    ...
                                    className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${selectedCategory === category ? 'bg-background text-accent shadow-premium' : 'text-muted-text hover:text-foreground'}`}
                                >
                                    {category}
                                </button>
                            ))}
                        ...
                        <div className="relative w-full md:w-96">
                            <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-text" />
                            <input 
                                type="text"
                                placeholder="Search articles..."
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="w-full pl-14 pr-6 py-4 bg-surface border border-border rounded-2xl focus:outline-none focus:ring-4 focus:ring-accent/5 focus:border-accent transition-all text-sm font-bold placeholder:font-black placeholder:uppercase placeholder:tracking-widest"
                            />
                        </div>
                    </div>
                            ...
                            <Link key={i} href={`/blog/${post.slug}`} className="group block h-full">
                                <div className="bg-surface border border-border rounded-[2.5rem] overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 h-full flex flex-col">
                                    <div className="relative aspect-video overflow-hidden">
                                        ...
                                        />
                                        <div className="absolute top-4 left-4 px-4 py-2 bg-surface/90 backdrop-blur-md rounded-xl text-[10px] font-black uppercase tracking-widest text-accent flex items-center space-x-2">
                                    ...
                                    <div className="p-8 space-y-4 flex-grow">
                                        ...
                                        <p className="text-secondary-text text-sm leading-relaxed line-clamp-3">
                                            {post.description}
                                        </p>
                                    </div>
                                    <div className="px-8 pb-8 pt-4 flex items-center justify-between border-t border-border/50 mx-8">
                                        <div className="flex items-center space-x-2 text-[10px] font-black text-muted-text uppercase tracking-widest">
                    ...
                    {filteredPosts.length === 0 && (
                        <div className="text-center py-24">
                            <p className="text-secondary-text text-xl font-bold">No articles found matching your criteria.</p>
                            <button 
